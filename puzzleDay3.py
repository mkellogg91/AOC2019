...
def plotLine(lineData, lineResultArray):
    ...
    # the first plot will affect the second plot
    xVal = 1
    yVal = 1

    for item in lineData:
        #grab the direction to move and distance
        MoveDir = item[0]
        MoveDis = int(item[1:])
        
        # loop for number of times to move in a direction
        for x in range(0, MoveDis):
...
# takes an array that the results should be spit out into
# an intersection occurs when both sets contain the same two x-y coords such as [0, 4] [0, 4]
def findIntersections(path1, path2, intersectionResultArray):

    # loop through one array searching for its coords in the other array and append results to a new array
...
# the central port using "Manhattan Distance"
def findClosestIntersection(interesections):
    shortestDistance = 0

    for item in interesections:
        # distance is equal to (centralport[x] - intersection[x]) + (centralport[y] - intersection[y])
        distanceCalc = abs(item[0] - 1) + abs(item[1] - 1) 
        
        if shortestDistance == 0 or distanceCalc < shortestDistance:
...
# calculate distance from intersections to central port and return the shortest distance
answer = findClosestIntersection(intersectionsList)
print("answer here:")
print(answer)
